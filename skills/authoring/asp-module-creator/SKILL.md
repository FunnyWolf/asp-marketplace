---
name: asp-module-creator
description: "创建 ASP 自定义告警处理模块。适用于根据 SIEM rule 样本和项目约定生成 custom/modules 模块。"
argument-hint: "为 <rule-name> 创建 ASP module"
...
metadata:
  ...
  tags: [module, alert-processing, development, SIEM]
...
- 编写代码前必须读取当前后端枚举模型，所有枚举值只能使用实际模型定义中的值，不得凭记忆或推断自行发明。
...
2. 如果 raw_alert 样本未获取，按优先级尝试三种方式获取（见 SOP 步骤 3）。
3. 获取样本后分析字段结构，再编写代码。
...
### 步骤 3 — 获取 raw_alert 样本

按以下优先级尝试，任意一种成功即可继续：

**方式 A（推荐，需已连接 ASP CLI）：**
调用 `asp dev stream head "<rule-name>" -n 3 --output json` 读取 stream 头部若干条告警。
或调用 `asp dev stream read "<rule-name>" <message_id> --output json` 读取指定消息。
...
**前置动作：** 读取当前后端枚举模型（`apps.alerts.models`、`apps.artifacts.models`、`apps.cases.models`、`apps.enrichments.models`），确认所有需要用到的 enum 合法值，再开始写代码。
...
- 如果无法通过 ASP CLI 读取 stream，询问用户选择方式 B 或方式 C 获取样本。
...
- 如果 ASP CLI 命令返回连接错误或超时，直接回复失败，提示用户运行 `asp doctor --output json`，检查 API URL、API key、网络连通性、用户状态和权限。不要尝试绕过认证。
- 如果无法连接 ASP CLI 且用户也无法提供 raw_alert 样本，说明无法继续并指引用户先完成前置条件。
