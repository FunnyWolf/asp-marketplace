...
- `time_range_start` 和 `time_range_end`：UTC 或带时区的 ISO 8601 采样时间范围。
...
调用 `siem_discover_index_fields(index_name=<index_name>, backend=<backend>, time_range_start=<start>, time_range_end=<end>, max_samples_per_field=20)`。

MCP 工具契约：
- `siem_discover_index_fields(index_name, backend, time_range_start, time_range_end, doc_limit=10000, max_samples_per_field=20)`
  - `backend` 必须是 `ELK` 或 `Splunk`。
  - `time_range_start` 和 `time_range_end` 是必填参数，必须是带时区的 ISO 8601，例如 `2026-06-23T12:00:00Z`。
  - `doc_limit` 后端限制为 1-100000。
  - `max_samples_per_field` 后端限制为 1-100。
...
- 只有在缺失时才询问 `index_name`、`backend` 或采样时间范围。
