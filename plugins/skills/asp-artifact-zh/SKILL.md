...
1. 如果用户要审查具体某个 artifact，调用 `list_artifacts(artifact_id=<id>, limit=1, include_related=True, include_comments=True)`。
2. 如果用户要浏览或对比 artifact，调用 `list_artifacts(..., include_related=False, include_comments=False)`。
...
1. 从请求中提取最窄且最有用的过滤条件。
2. 广泛查询/列表使用 `include_related=False, include_comments=False`；只有具体某条 artifact 需要 enrichment 上下文时才用 `include_related=True`，需要评论时才用 `include_comments=True`。
3. 调用 `list_artifacts`。
4. 解析返回的 JSON 字符串。
