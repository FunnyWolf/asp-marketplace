...
5. Generate SPL or ES|QL detection rules based on the detection target and available fields.
6. Call `siem_execute_spl` or `siem_execute_esql` to validate the rule.
7. Adjust the rule based on validation results.
8. Map to MITRE ATT&CK tactics and techniques.
9. Output the final rule and usage instructions.
