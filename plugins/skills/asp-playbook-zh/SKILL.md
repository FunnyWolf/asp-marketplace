...
- `list_playbook_definitions` 只用于查询可运行的 definition。
...
1. 如果用户想知道哪些 playbook 可以运行，调用 `list_playbook_definitions`。
2. 如果用户想确认某个 case 是否执行过自动化，调用 `list_playbook_runs(case_id=<case_id>)`。
3. 如果用户要执行自动化，并已提供 definition 名称和目标 case，调用 `execute_playbook`。
4. 如果用户想执行自动化但不知道 definition 名称，先调用 `list_playbook_definitions`。
...
1. 调用 `list_playbook_definitions`。
2. 解析返回的 JSON。
...
2. 如果 definition 名称缺失或不确定，先调用 `list_playbook_definitions`。
